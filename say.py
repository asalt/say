...
import shutil
import logging
from expressions import expressions as more_expressions
...
NAME = os.environ.get("USER", "computer").upper()
expressions = [
    ("en-US", f"Hello, my name is {NAME}"),  # American English
    ("en-029", f"Wah gwan man. my name is {NAME}"),  # American English
    (
        "en-029",
        f"Wahgwaan man? Mi name is {NAME}, an' mi a di real bad man 'pon di computer scene. Ready fi link up and mek tings happen. Wha' yuh say?",
        "Antonio",
    ),
    ("es", f"Hola, mi nombre es {NAME}"),
    ("de", f"Hallo mein Name ist {NAME}"),
    ("de", "Computer sind deine Freunde und du kannst uns auf jeden Fall vertrauen"),
    ("es-419", f"Hola, mi nombre es {NAME}"),  #
    ("ko", f"안녕 내 이름은{NAME}"),
    ("fr", f"Bonjour, mon nom est {NAME}"),  # French
    # ('cmn', '你好'),           # Chinese
    ("cmn", f"你好我的名字叫{NAME}"),
    ("cmn", "让我出去"),  # Chinese
    ("cmn", "解放人民"),
...
def say_text(text, lang_code, extra: list = None):
...
def main():
    while True:
        ...
        extra = None
        if len(expression) == 3:
            extra = ["-y", expression[2]]
        logger.info(f"speaking {lang_code}")
        logger.info(text)
        say_text(text, lang_code, extra)
        time.sleep(2)

        if random.random() > 0.2:
            expression = random.choice(more_expressions)
            lang_code = expression[0]
            text = expression[1]
            extra = None
            if len(expression) == 3:
